			...
			struct node_info *ni)
{
	...
	if (!IS_VALID_BLK_ADDR(sbi, ni->blk_addr)) {
		ASSERT_MSG("blkaddress is not valid. ni->ino [0x%x], blkaddr [0x%x]", ni->ino, ni->blk_addr);
		return -EINVAL;
	}
...
static void print_disk_space_usage(struct f2fs_sb_info *sbi, u32 nid, u8 type)
{
	...
	if (!IS_VALID_BLK_ADDR(sbi, ni.blk_addr) || ni.blk_addr == 0) {
...
int fsck_verify(struct f2fs_sb_info *sbi)
{
	...
	printf("[FSCK] Max image size: %"PRIu64" MB, Free space: %lu MB\n",
