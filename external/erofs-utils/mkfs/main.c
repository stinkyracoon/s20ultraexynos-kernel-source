...
static struct option long_options[] = {
	...
	{"proca-attrs", required_argument, NULL, 15},
#ifdef WITH_ANDROID
	{"mount-point", required_argument, NULL, 512},
	{"product-out", required_argument, NULL, 513},
	{"fs-config-file", required_argument, NULL, 514},
	{"block-list-file", required_argument, NULL, 515},
	{"du-list-file", required_argument, NULL, 768},
#endif
	{0, 0, 0, 0},
};
...
static void usage(void)
{
	fputs("usage: [options] FILE DIRECTORY\n\n"
...
#endif
	      " --proca_attrs=X        specify a file to setup proca labels\n"
	      ...
	      " --du-list-file=X      X=du_liste file\n"
#endif
...
static int mkfs_parse_options_cfg(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "C:E:T:U:d:x:z:",
		...
		case 14:
			...
			break;
		case 15:
			cfg.xattr_table = xattr_table_open(optarg);
			if (!cfg.xattr_table) {
				erofs_dbg("failed to load xattr_table_file: %s",
					optarg);
				return -EINVAL;
			}
			break;
...
int main(int argc, char **argv)
{
	...
	if (cfg.du_list_file) {
		err = erofs_droid_dulist_fopen();

		if (err) {
			erofs_err("failed to open %s", cfg.du_list_file);
			goto exit;
		}
	}
#endif
